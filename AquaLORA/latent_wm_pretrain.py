...
def PRVL_loss(img1, img2):
    ...
    diff_sum = F.conv2d(diff_combined, KERNEL, padding=WINDOW_SIZE // 2).squeeze(
        0
    )  # [1, 513, 513]
...
def main(args):
    ...
    optimizer = optim.AdamW(
        [{"params": sec_encoder.parameters()}, {"params": sec_decoder.parameters()}],
        lr=args.lr,
        weight_decay=1e-4,
    )
    ...
    def gen_combined_latents(latents, wm_latent, scale=1.0):
        cornerfy_aug = random.choice(
            [True, False, False, False]
        )  # 1/4 chance to cornerfy_aug
        ...
        if cornerfy_aug:
            ...
            wm_template[:, :, : height // 2, : width // 2] = wm_latent[
                :, :, : height // 2, : width // 2
            ]
            wm_template[:, :, : height // 2, -width // 2 :] = wm_latent[
                :, :, : height // 2, -width // 2 :
            ...
            wm_template = F.interpolate(
                wm_template, size=(height, width), mode="bilinear"
            )
        else:
    ...
    for epoch in range(current_epoch, current_epoch + args.epochs):
        ...
        for batch_idx, oimage in enumerate(train_loader):
            ...
            msg = torch.randint(0, 2, (args.batch_size, args.bit_num)).cuda() # multiple_messages
            
            print("Message is", msg)
            
            _, wm_latent = sec_encoder(latents, msg.float())
            
            print("wm_latent", wm_latent.shape)

            if warmup:
                watermarked_latents = gen_combined_latents(
                    latents, wm_latent, scale=0.03
                )
            else:
            ...
            if epoch - current_epoch > 12 or args.resume_from_ckpt is not None:
                watermarked_image = noiser(
                    [watermarked_image, None], [0.4, 0.1, 0.2, 0.05, 0.1, 0.15]
                )[0]
            else:
                watermarked_image = noiser(
                    [watermarked_image, None], [0.6, 0.0, 0.4, 0.0, 0.0, 0.0]
                )[0]
            ...
            iterations += 1
            print(
                "lpips_loss: %.4f, msgloss: %.4f, prvl_loss: %.4f, loss: %.4f"
                % (lpips_loss, msgloss, prvl_loss, loss)
            )
        ...
        watermarked_image_pil.save(
            f"{args.output_dir}/log_images/watermarked_{epoch}_{batch_idx}.png"
        )
        ...
        with torch.no_grad():
            ...
            acc = 1 - torch.abs(decoded_msg - msg_val).sum().float() / (
                args.bit_num * args.batch_size
            )
            print(f"Epoch {epoch}: acc {acc}")
            writer.add_scalar("Accuracy/train", acc, epoch)

        print(
            f"Epoch {epoch}: loss {loss}, lpips_loss {lpips_loss}, msgloss {msgloss}, prvl_loss {prvl_loss}"
        )
